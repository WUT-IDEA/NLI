...
#         print(len(outputs)) #2
#         print(outputs[0].shape) #torch.Size([16, 100, 768]) 100是max_len
#         print(outputs[1]) #None
#         print(len(outputs[2])) #13
        hidden_states = outputs[2]
    #         hidden_states：这是输出的一个可选项，如果输出，需要指定config.output_hidden_states=True,它也是一个元组，它的第一个元素是embedding，其余元素是各层的输出，每个元素的形状是(batch_size, sequence_length, hidden_size)
#         print('0',hidden_states[0])
#         for i in range(len(outputs)):
#             if torch.is_tensor(outputs[i]):
...
#         logits = self.classifier(sequence_output)
        ...
        for i in range(len(hidden_states)):
            sequence_output.append(hidden_states[i])
#         print('sequence_output len is:', len(sequence_output))
        logits, score= self.classifier(sequence_output)
#         if torch.is_tensor(logits):
#             print('logits len is:', logits.shape)
#         elif isinstance(logits,tuple):
#             print('logits len is:', len(logits))
#         else:
#             print('logits len is:', logits)
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.query = nn.Linear(config.hidden_size*12, 12)#768*12 *768=9216*12 实质是attention matrix 其实是加了个全连接层 y=wx
        self.value = nn.Linear(config.hidden_size*12, config.hidden_size*12)  # 512->5129216*9216
...
#         print('len(features[1:]features): ',len(features))
#         logger.info("len(features):  is %s", len(features))
#         print('sequence_output.shape: ',sequence_output.shape)
#         print('len(features): ',len(features))
#         print('sequence_output.shape: ',sequence_output.shape)
#         logger.info("sequence_output.shape:  is %s", sequence_output.shape)
#         print('score.shape:',score.shape)
#         logger.info("score.shape:  is %s", score.shape)
#         print('unsqueeze score.shape:',score.shape)
#         logger.info("unsqueeze score.shape:  is %s", score.shape)
#         print('softmax score.shape:',score.shape)
#         logger.info("softmax score.shape:  is %s", score.shape)
#         logger.info("softmax score:  is %s", score)
#         print('value.shape:',value.shape)
#         logger.info("value.shape:  is %s",value.shape)
#         print('sequence_output.size(0):',size)
#         logger.info("sequence_output.size(0) %s",size)
#         print('value.reshape:',value.shape)
#         logger.info("value.reshape: %s",value.shape)
        
#         value = torch.reshape(value, ((16, 12, 768)))
#         print('value.transpose(-1, -2):',value.transpose(-1, -2).shape)
#         value = torch.matmul(score, value.transpose(-1, -2))
#         print('value.shape:',value.shape)
#         logger.info("value.shape: %s",value.shape)
#         print('x.shape:',x.shape)
#         logger.info("x.out_proj: %s",x.shape）
        features = features[1:] #只取了后12个 也就是12层的输出
        sequence_output = features[0][:, 0, :] #16 ，max_len 768
        features = features[1:] #11 16 max_len 768
        for i in range(len(features)): #0,11 后11层
            sequence_output = torch.cat([sequence_output, features[i][:, 0, :]], dim=1)
        score = self.query(sequence_output)
        score = score.unsqueeze(1)
        f = nn.Softmax(dim = 2)
        score = f(score)
        value = self.value(sequence_output)
        size = sequence_output.size(0)
        value = value.reshape([size, 12, 768]);
        value = torch.matmul(score, value)
